...
-- Internal/test identities are excluded from graph edges so they don't cluster
-- test forms with internal correspondence. The exclusion list matches
-- INTERNAL_EMAIL_ADDRESSES in test-exclusion.ts and test_numbers in BQ.
CREATE TEMP TABLE contact_points AS
-- Spine events (phone edges, excluding test phones)
SELECT 'S' AS src, event_id, event_ts, phone AS cp, 'phone' AS cp_type FROM spine_events
WHERE phone IS NOT NULL AND phone != ''
  AND phone NOT IN (SELECT phone_e164 FROM `pttr-taskdata.ds_crm.test_numbers`)
UNION ALL
-- Spine events (email edges, excluding internal emails)
SELECT 'S', event_id, event_ts, email, 'email' FROM spine_events
WHERE email IS NOT NULL AND email != ''
  AND LOWER(email) NOT IN (SELECT email FROM `pttr-taskdata.ds_crm.test_emails`)
UNION ALL
